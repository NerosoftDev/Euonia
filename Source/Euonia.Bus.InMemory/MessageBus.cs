...
public abstract class MessageBus : DisposableObject, IMessageBus
{
    ...
    protected MessageBus(IMessageHandlerContext handlerContext, IServiceAccessor accessor)
    {
    ...
    protected virtual void OnMessageSubscribed(MessageSubscribedEventArgs args)
    {
        ...
        queue.MessagePushed += (sender, e) =>
        {
            ...
            OnMessageReceived(new MessageReceivedEventArgs(message, e.MessageContext));
        };
    ...
    /// <summary>
    /// Handles the <see cref="E:MessageDispatched" /> event.
    /// </summary>
