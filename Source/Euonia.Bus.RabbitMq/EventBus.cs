...
public class EventBus : MessageBus, IEventBus
{
        ...
        : base(handlerContext, monitor, accessor)
    {
        ...
        {
            throw new ArgumentNullException(nameof(Options.ExchangeName), "Exchange name is required");
        }

        if (string.IsNullOrEmpty(Options.ExchangeType))
        {
            throw new ArgumentNullException(nameof(Options.ExchangeType), "Exchange type is required");
        }
